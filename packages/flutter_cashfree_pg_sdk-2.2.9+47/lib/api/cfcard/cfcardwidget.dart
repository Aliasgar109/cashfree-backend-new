...
import '../cfsession/cfsession.dart' as local;
...
class CFCardWidget extends StatefulWidget {
  ...
  final local.CFSession? cfSession;
...
class CFCardWidgetState extends State<CFCardWidget> {
  ...
  CFCardWidgetState(InputDecoration? inputDecoration, TextStyle? textStyle, Function(CFCardListener) cardListener, local.CFSession cfSession) {
  ...
  Future<void> _handleTextChanged(String newText) async {
    ...
    if(textWithoutSpaces.length == 8) {
      ...
      var tdrResponse = await CFNetworkManager().getTDR(_cfSession! as dynamic, textWithoutSpaces);
      var cardbinResponse = await CFNetworkManager().getCardBin(_cfSession! as dynamic, textWithoutSpaces);
      _tdrJson = null;
      _cardbinJson = null;
      ...
      if(_first_eight_digits == textWithoutSpaces.substring(0, 8)) {
        ...
        var tdrResponse = await CFNetworkManager().getTDR(_cfSession! as dynamic, _first_eight_digits);
        var cardbinResponse = await CFNetworkManager().getCardBin(_cfSession! as dynamic, _first_eight_digits);
        _tdrJson = null;
        _cardbinJson = null;
        if(tdrResponse.statusCode == 200) {
          _tdrJson = json.decode(tdrResponse.body);
