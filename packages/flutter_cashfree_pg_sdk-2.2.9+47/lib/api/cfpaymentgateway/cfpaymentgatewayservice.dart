...
import 'package:flutter_cashfree_pg_sdk/api/cferrorresponse/cferrorresponse.dart' as local;
...
class CFPaymentGatewayService {
  ...
  static void Function(local.CFErrorResponse, String)? onError;

  setCallback(final void Function(String) vp, final void Function(local.CFErrorResponse, String) error) {
    verifyPayment = vp;
    onError = error;
    ...
    methodChannel.invokeMethod("response").then((value) {
      if(value != null) {
        ...
        switch (status) {
          ...
          case "failed":
            var data = body["data"] as Map<String, dynamic>;
            var errorResponse = local.CFErrorResponse(
  ...
  doPayment(CFPayment cfPayment) {
    ...
    if(cfPayment is CFCardPayment) {
      CFCardPayment cfCardPayment = cfPayment as CFCardPayment;
      ...
      if (cfPayment is CFDropCheckoutPayment) {
        ...
        CFWebCheckoutPayment webCheckoutPayment = cfPayment as CFWebCheckoutPayment;
        data = _convertToWebCheckoutMap(webCheckoutPayment);
      } else if(cfPayment is CFUPIPayment) {
        CFUPIPayment cfupiPayment = cfPayment as CFUPIPayment;
        data = _convertToUPItMap(cfupiPayment);
      } else if (cfPayment is CFNetbankingPayment) {
        CFNetbankingPayment cfNetbankingPayment = cfPayment as CFNetbankingPayment;
        data = _convertToNetbankingMap(cfNetbankingPayment);
      } else if (cfPayment is CFSubscriptionPayment) {
        CFSubscriptionPayment cfSubscriptionPayment = cfPayment as CFSubscriptionPayment;
      ...
      if (cfPayment is CFDropCheckoutPayment) {
        ...
        CFUPIPayment upiPayment = cfPayment as CFUPIPayment;
        if(upiPayment.getUPI().getChannel() == CFUPIChannel.INTENT_WITH_UI) {
  ...
  void responseSubscriptionMethod(dynamic value) {
    if(value != null) {
      ...
      switch (status) {
        case "exception":
          var data = body["data"] as Map<String, dynamic>;
          var cfErrorResponse = local.CFErrorResponse("FAILED", data["message"] as String, "invalid_request", "invalid_request");
  ...
  void _completePaymentWithInstrumentId(final void Function(String) verifyPayment,
      final void Function(local.CFErrorResponse, String) onError,
      String card_cvv,
      Map<String, dynamic> session,
  ...
  _createErrorResponse(String? message, String? code, String? type, String? orderId) {
    var cfErrorResponse = local.CFErrorResponse("FAILED", message ?? "something went wrong", code ?? "invalid_request", type ?? "invalid_request");
